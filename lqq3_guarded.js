const USE_WORKER_LIVE = false;
  const WORKER_QUOTE_URL = "https://spx-quote-proxy.rkarim88.workers.dev/?mode=quote&symbol=lqq3";
  const ASSET_LABEL = "LQQ3";
  const EXPECTED_TICKER = "LQQ3.L";
  const STATIC_DAILY_URL = "lqq3_daily.csv";
  const STATIC_SIGNAL_URL = "latest_lqq3_signal.json";
  const STATIC_SITE_DATA_URL = "lqq3_guarded_site_data.json";
    ...
    if ($("backtestCallout") && bt && bh) {
      ...
      if (original) {
        $("backtestCallout").textContent =
          `On ${data.asset_label || "LQQ3.L"} (${sample?.start_date || "?"} to ${sample?.end_date || "?"}), ` +
        ...
        $("backtestCallout").textContent =
          `On ${data.asset_label || "LQQ3.L"}, Guarded A5/B25 delivered ${bt.cagr_pct} CAGR and ${bt.max_drawdown_pct} max drawdown ` +
  ...
  function updateAutoRefreshStatus(prefix = "") {
    ...
    parts.push("Enter a manual LQQ3 (LQQ3.L) level then click refresh to override the live quote.");
  ...
  function selectedWindowPnlData() {
    ...
    return {
      points,
      meta: `Starts ${first.date} at ${entryLabel} (${startReason}); chart rebased to 0% at window start. Strategy ${fmtSignedPct(windowReturn)} vs LQQ3 3x Nasdaq ${fmtSignedPct(benchmarkReturn)} through ${last.date}. ${costLabel}.`,
    };
  }
  ...
  function renderBacktestEquityChart() {
    ...
    $("equityChartMeta").textContent = `${latestRowsSourceLabel()}; ${data.length} trading sessions from ${startDate} to ${endDate}. Cumulative return rebased to 0% at range start: strategy ${fmtSignedPct(last.strategyReturn)} vs LQQ3 3x Nasdaq buy &amp; hold ${fmtSignedPct(last.spxReturn)}.`;
  }
