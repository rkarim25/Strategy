const WORKER_DAILY_URL = "https://spx-quote-proxy.rkarim88.workers.dev/?mode=daily&symbol=ndx";
  const WORKER_QUOTE_URL = "https://spx-quote-proxy.rkarim88.workers.dev/?mode=quote&symbol=ndx";
  ...
  async function loadStaticHistoricalBacktest() {
    try {
      ...
      if (Number.isFinite(staticQuotePrice) && staticQuotePrice > 0) {
        ...
        liveResult.explanation = `Scheduled static quote fallback from ${signalMetadata.quote_source || "latest_ndx_signal.json"}${quoteTime ? ` (${quoteTime})` : ""}. This intraday signal remains provisional.`;
      }
      ...
      updateRangeNudgeStates();
      setStatus(`Preloaded ${rows.length} NDX sessions (${rows[0].date} to ${rows[rows.length - 1].date}).${siteData?.generated_at_utc ? " Metrics through " + siteData.sample?.end_date + "." : ""}${describeStaticSignalMetadata(signalMetadata)} Auto-refresh will check live data next.`);
    } catch (err) {
      console.error(err);
  ...
  async function loadStaticSignalMetadata() {
    try {
      const { text } = await fetchTextWithDiagnostics(STATIC_SIGNAL_URL, "Static NDX signal metadata");
  ...
  async function loadStaticSiteData() {
    try {
      const { text } = await fetchTextWithDiagnostics(STATIC_SITE_DATA_URL, "NDX site data");
  ...
  async function refresh(options = {}) {
    ...
    refreshInFlight = (async () => {
      try {
        setStatus(`${reason === "manual" ? "Manual refresh" : "Auto-refresh"}: fetching daily Nasdaq 100 data...`);
        const { text: csv } = await fetchTextWithDiagnostics(WORKER_DAILY_URL, "Daily NDX data");
        ...
        setStatus(`${reason === "manual" ? "Manual refresh" : "Auto-refresh"}: fetching live NDX quote...`);
  ...
  async function getLivePrice(options = {}) {
    ...
    const { response, text } = await fetchTextWithDiagnostics(WORKER_QUOTE_URL, "Intraday NDX quote");
    let parsedJson = null;
    try {
    ...
    for (let i = 0; i < rows.length; i++) {
      ...
      if (regime === "tier3") {
        if (close / entryClose - 1 >= params.hold3) {
          ...
          targetLeverage = recoveryOk ? 3 : baseLev;
      ...
      if (regime === "tier2") {
        ...
        if (close / entryClose - 1 >= params.hold2) {
          ...
          targetLeverage = recoveryOk ? 2 : baseLev;
          explanation = recoveryOk
    ...
    for (let i = 0; i < rows.length; i++) {
      ...
      if (regime === "tier3") {
        ...
        if (hitGain || hitTime) {
          ...
          leverage.push(recoveryOk ? 3 : baseLev);
          continue;
        }
      }
      ...
      if (regime === "tier2") {
        ...
        if (hitGain || hitTime) {
          ...
          leverage.push(recoveryOk ? 2 : baseLev);
          continue;
        }
      }
