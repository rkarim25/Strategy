"""Generic multi-asset mega-script: comprehensive strategy backtests across ALL 10 asset classes.

Runs ~35 strategies per asset, outputs per-asset CSV + combined CSV.
Self-contained signal functions; imports only engine/metrics/etp_leverage from the codebase.
"""
...
OUTPUT_DIR = ROOT / "output" / "strategy_results"
...
# ===================================================================
# PART 1: ASSET CLASS DEFINITIONS
# ===================================================================
...
# ===================================================================
# PART 2: INDICATOR FUNCTIONS (self-contained)
# ===================================================================
...
# ===================================================================
# PART 4: ETP RETURN PANEL CONSTRUCTION
# ===================================================================
...
# ===================================================================
# PART 5: SIGNAL FUNCTIONS (self-contained, generic)
# ===================================================================
...
# ===================================================================
# PART 6: STRATEGY UNIVERSE BUILDER
# ===================================================================
...
def build_strategies(prices: pd.DataFrame, asset_key: str) -> list[tuple[str, pd.Series, float]]:
    ...
    # ===================================================================
    # 3x Strategies (only for max_leverage >= 3)
    # ===================================================================
    if max_lev >= 3:
    ...
    # ===================================================================
    # Counter-Cyclical / Scaled Leverage (1-3x, capped at max_leverage)
    # ===================================================================
    cc_max = min(3, max_lev)
    cc_label = f"1-{cc_max}x"
    ...
    add(f"SMA200 ±3% Band + RSI Scale {cc_label}", hybrid_rsi_exit, float(cc_max))

    # SMA200 ±3% Band + VIX Scale
    ...
    add(f"SMA200 ±3% Band + VIX Scale {cc_label}", hybrid_vix_exit, float(cc_max))

    # SMA200 ±3% Band + Distance Scale
    dist_scale = distance_leverage_scale(band_2x, prices, 200)
    dist_scale = dist_scale.clip(upper=float(cc_max))
    add(f"SMA200 ±3% Band + Distance Scale {cc_label}", dist_scale, float(cc_max))

    # SMA200 ±3% Band + DD Scale
    ...
    add(f"SMA200 ±3% Band + DD Scale {cc_label}", hybrid_dd, float(cc_max))

    # SMA200 ±3% Band + BB Scale
    ...
    add(f"SMA200 ±3% Band + BB Scale {cc_label}", hybrid_bb, float(cc_max))

    # Standalone RSI Counter-Cyclical
    rsi_cc_standalone = rsi_cc.clip(upper=float(cc_max))
    add(f"RSI Counter-Cyclical {cc_label}", rsi_cc_standalone, float(cc_max))

    # Standalone VIX Counter-Cyclical
    vix_cc_standalone = vix_cc.clip(upper=float(cc_max))
    add(f"VIX Counter-Cyclical {cc_label}", vix_cc_standalone, float(cc_max))
...
def main() -> int:
    print("=" * 80)
    print("MULTI-ASSET STRATEGY SWEEP — ALL 10 ASSET CLASSES")
    print("=" * 80)
    ...
    for asset_idx, asset_key in enumerate(asset_keys):
        ...
        # --- Write per-asset CSV ---
        csv_path = OUTPUT_DIR / f"{asset_key}_results.csv"
        _write_results_csv(asset_results, csv_path)
        print(f"  -> Wrote {len(asset_results)} results to {csv_path.name}")

    # -------------------------------------------------------------------
    # 3. Write combined CSV
    # -------------------------------------------------------------------
    if all_results:
        combined_path = OUTPUT_DIR / "all_assets_combined.csv"
        _write_results_csv(all_results, combined_path, include_asset=True)
        print(f"\n{'=' * 80}")
