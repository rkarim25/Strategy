const USE_WORKER_LIVE = false;
  const WORKER_QUOTE_URL = "https://spx-quote-proxy.rkarim88.workers.dev/?mode=quote&symbol=msci_world";
  const ASSET_LABEL = "MSCI World";
  const EXPECTED_TICKER = "SWDA.L";
  const STATIC_DAILY_URL = "msci_world_daily.csv";
  const STATIC_SIGNAL_URL = "latest_msci_world_signal.json";
  const STATIC_SITE_DATA_URL = "msci_world_guarded_site_data.json";
  ...
  async function loadStaticHistoricalBacktest() {
    try {
      ...
      if (Number.isFinite(staticQuotePrice) && staticQuotePrice > 0) {
        ...
        liveResult.explanation = `Scheduled static quote fallback from ${signalMetadata.quote_source || "latest_msci_world_signal.json"}${quoteTime ? ` (${quoteTime})` : ""}. This intraday signal remains provisional.`;
      }
    ...
    if ($("backtestCallout") && bt && bh) {
      ...
      if (original) {
        $("backtestCallout").textContent =
          `On ${data.asset_label || "SWDA.L"} (${sample?.start_date || "?"} to ${sample?.end_date || "?"}), ` +
        ...
        $("backtestCallout").textContent =
          `On ${data.asset_label || "SWDA.L"}, Guarded A5/B25 delivered ${bt.cagr_pct} CAGR and ${bt.max_drawdown_pct} max drawdown ` +
  ...
  function updateAutoRefreshStatus(prefix = "") {
    ...
    parts.push("Enter a manual MSCI World (SWDA.L) level then click refresh to override the live quote.");
  ...
  async function getLivePrice(options = {}) {
    ...
    const { response, text } = await fetchTextWithDiagnostics(WORKER_QUOTE_URL, "Intraday MSCI World quote");
    let parsedJson = null;
    try {
  ...
  function selectedWindowPnlData() {
    ...
    return {
      points,
      meta: `Starts ${first.date} at ${entryLabel} (${startReason}); chart rebased to 0% at window start. Strategy ${fmtSignedPct(windowReturn)} vs MSCI World ${fmtSignedPct(benchmarkReturn)} through ${last.date}. ${costLabel}.`,
    };
  }
  ...
  function renderBacktestEquityChart() {
    ...
    $("equityChartMeta").textContent = `${latestRowsSourceLabel()}; ${data.length} trading sessions from ${startDate} to ${endDate}. Cumulative return rebased to 0% at range start: strategy ${fmtSignedPct(last.strategyReturn)} vs MSCI World buy &amp; hold ${fmtSignedPct(last.spxReturn)}.`;
  }
