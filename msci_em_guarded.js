const USE_WORKER_LIVE = false;
  ...
  async function loadStaticHistoricalBacktest() {
    try {
      ...
      if (Number.isFinite(staticQuotePrice) && staticQuotePrice > 0) {
        ...
        liveResult.explanation = `Scheduled static quote fallback from ${signalMetadata.quote_source || "latest_msci_em_signal.json"}${quoteTime ? ` (${quoteTime})` : ""}. This intraday signal remains provisional.`;
      }
    ...
    if ($("backtestCallout") && bt && bh) {
      ...
      if (original) {
        $("backtestCallout").textContent =
          `On ${data.asset_label || "EEM"} (${sample?.start_date || "?"} to ${sample?.end_date || "?"}), ` +
        ...
        $("backtestCallout").textContent =
          `On ${data.asset_label || "EEM"}, Guarded A5/B25 delivered ${bt.cagr_pct} CAGR and ${bt.max_drawdown_pct} max drawdown ` +
  ...
  async function getLivePrice(options = {}) {
    ...
    const { response, text } = await fetchTextWithDiagnostics(WORKER_QUOTE_URL, "Intraday MSCI EM (EEM) quote");
    let parsedJson = null;
    try {
