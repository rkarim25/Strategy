const USE_WORKER_LIVE = false;
  ...
  const STATIC_SITE_DATA_URL = "3bal_guarded_site_data.json";
  ...
  function smaCashLeverageForRows(rows) {
    const closes = rows.map((row) => row.close);
    const leverage = [];
    for (let i = 0; i < rows.length; i++) {
      const avg20 = sma(closes, i, DEFAULT_STRATEGY.smaWindow);
      leverage.push(Number.isFinite(avg20) && closes[i] > avg20 ? 1 : 0);
    }
    return { leverage };
  }

  function backtestDefaultStrategy(rows) {
    const { leverage } = smaCashLeverageForRows(rows);
    let aum = INITIAL_CAPITAL;
    let prevLev = leverage[0] ?? 0;
    let prevYear = new Date(rows[0].date).getUTCFullYear();
    let tradingCosts = 0;
    let rebalances = 0;
    const equity = [];
    const returns = [];
    const dailyCash = DEFAULT_STRATEGY.cashRate / TRADING_DAYS;
    ...
    for (let i = 0; i < rows.length; i++) {
      ...
      let ret = 0;
      if (i > 0) {
        const assetRet = rows[i].close / rows[i - 1].close - 1;
        ret = lev <= 0 ? dailyCash : lev * assetRet;
        aum *= 1 + ret;
      }
  ...
  async function loadStaticHistoricalBacktest() {
    try {
      ...
      if (Number.isFinite(staticQuotePrice) && staticQuotePrice > 0) {
        ...
        liveResult.explanation = `Scheduled static quote fallback from ${signalMetadata.quote_source || "latest_lqq3_signal.json"}${quoteTime ? ` (${quoteTime})` : ""}. This intraday signal remains provisional.`;
      }
      ...
      updateRangeNudgeStates();
      setStatus(`Preloaded ${rows.length} 3BAL sessions (${rows[0].date} to ${rows[rows.length - 1].date}).${siteData?.generated_at_utc ? " Metrics through " + siteData.sample?.end_date + "." : ""}${describeStaticSignalMetadata(signalMetadata)} Auto-refresh will check live data next.`);
    } catch (err) {
      console.error(err);
  ...
  async function loadStaticSignalMetadata() {
    try {
      const { text } = await fetchTextWithDiagnostics(STATIC_SIGNAL_URL, "Static 3BAL signal metadata");
  ...
  async function loadStaticSiteData() {
    try {
      const { text } = await fetchTextWithDiagnostics(STATIC_SITE_DATA_URL, "3BAL site data");
  ...
  function applySiteData(data) {
    ...
    if ($("backtestCallout") && bt && bh) {
      $("backtestCallout").textContent =
        `On ${data.asset_label || "3BAL.L"} (${sample?.start_date || "?"} to ${sample?.end_date || "?"}), ` +
        `SMA20 1x/cash delivered ${bt.cagr_pct} CAGR and ${bt.max_drawdown_pct} max drawdown vs buy-and-hold ` +
        `${bh.cagr_pct} / ${bh.max_drawdown_pct}.`;
    }
    ...
    if (diag && bt) {
      const cashPct = bt.pct_days_cash != null ? `${Number(bt.pct_days_cash).toFixed(2)}%` : "-";
      diag.innerHTML = `
        <tr><td>Cash days</td><td>${cashPct}</td><td>Invested days</td><td>${cashPct !== "-" ? `${(100 - Number(bt.pct_days_cash)).toFixed(2)}%` : "-"}</td></tr>
        <tr><td>Rebalances</td><td>${bt.rebalances ?? "-"}</td><td>Sharpe</td><td>${bt.sharpe_fmt ?? "-"}</td></tr>
        <tr><td>Total trading costs</td><td colspan="3">${bt.trading_costs_total != null ? fmtCompactCurrency(bt.trading_costs_total) : "-"}</td></tr>
      `;
    }

    const mcBody = $("mcComparisonBody");
    if (mcBody && mc && Array.isArray(data.comparison_table)) {
      mcBody.innerHTML = data.comparison_table.map((row) => {
        ...
        return `<tr>
          <td>${strong}${row.strategy}${strongEnd}</td>
          <td>${isDefault ? strong + (mc.median_cagr_pct || "-") + strongEnd : row.cagr_pct || "-"}</td>
  ...
  async function refresh(options = {}) {
    ...
    refreshInFlight = (async () => {
      try {
        setStatus(`${reason === "manual" ? "Manual refresh" : "Auto-refresh"}: reloading static ${ASSET_LABEL} data...`);
  ...
  function updateAutoRefreshStatus(prefix = "") {
    ...
    parts.push("Enter a manual 3BAL level then click refresh to override the live quote.");
  ...
  function computeSignal(rows) {
    ...
    for (let i = 0; i < rows.length; i++) {
      ...
      explanation = aboveSma
        ...
        : "Close is below SMA20; hold cash.";
    }

    const lastIndex = rows.length - 1;
    const latest = rows[lastIndex];
    const latestSma = sma(closes, lastIndex, DEFAULT_STRATEGY.smaWindow);
    const latestDd = latest.close / highWater - 1;
    const activeEntryClose = targetLeverage <= 0 ? null : baseEntryClose;
    const activeEntryDate = targetLeverage <= 0 ? null : baseEntryDate;
    ...
    return {
      ...
      recoveryOk: latest.close > latestSma,
      targetLeverage,
      activeEntryClose,
      activeEntryDate,
      activeEntryLeverage,
  ...
  function render(eod, live) {
    ...
    $("tier2Level").textContent = "N/A (SMA20 default)";
    $("tier3Level").textContent = "N/A (SMA20 default)";
    $("aboveSma").textContent = eod.aboveSma ? "Yes" : "No";
    $("leadGuardLevel").textContent = Number.isFinite(eod.latestSma)
      ? fmtMoney(eod.latestSma)
      : "-";
  }
  ...
  function computeStrategyDrawdownEpisodes() {
    if (!latestRows.length) return [];
    const equity = backtestDefaultStrategy(latestRows).equity;
    const episodes = [];
  ...
  function chartDataForRange() {
    ...
    const leverage = smaCashLeverageForRows(latestRows).leverage;
  ...
  function selectedWindowPnlData() {
    ...
    for (let i = 1; i < active.length; i++) {
      ...
      if (Math.abs(lev - prevLev) > 1e-9) {
        const cost = Math.abs(lev - prevLev) * strategy * DEFAULT_STRATEGY.tradingCost;
        strategy -= cost;
        tradingCosts += cost;
        rebalances += 1;
        prevLev = lev;
      ...
      const dailyReturn = lev <= 0 ? dailyCash : lev * spxReturn;
    ...
    return {
      points,
      meta: `Starts ${first.date} at ${entryLabel} (${startReason}); chart rebased to 0% at window start. Strategy ${fmtSignedPct(windowReturn)} vs 3BAL 3x ETP ${fmtSignedPct(benchmarkReturn)} through ${last.date}. ${costLabel}.`,
    };
  }
  ...
  function showSignalPnlTooltip(event) {
    ...
    tooltip.innerHTML = `
      ...
      3BAL buy &amp; hold: ${fmtSignedPct(nearest.spxReturn)}<br>
  ...
  function equityDataForRange() {
    if (!latestRows.length) return [];
    const strategy = backtestDefaultStrategy(latestRows).equity;
  ...
  function renderBacktestEquityChart() {
    ...
    $("equityChartMeta").textContent = `${latestRowsSourceLabel()}; ${data.length} trading sessions from ${startDate} to ${endDate}. Cumulative return rebased to 0% at range start: strategy ${fmtSignedPct(last.strategyReturn)} vs 3BAL 3x ETP buy &amp; hold ${fmtSignedPct(last.spxReturn)}.`;
  }
